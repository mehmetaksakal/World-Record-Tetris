/***********************************************************************
 * $Id:: canopen_driver.c 1604 2014-07-14 11:34:47     $
 *
...
CAN_CALLBACKS callbacks = { CAN_RX, /* callback for any message received CAN frame which ID matches with any of the message objects' masks */
...
void CANopen_1ms_tick(void) {
	...
	if (CANopen_Heartbeat_Producer_Value) {
		if ((++(CANopen_Heartbeat_Producer_Counter))
	...
	for (i = 0; i < WatchListLength; i++) {
		if (WatchList[i].value & 0x0000FFFF) {
			...
			if ((++(WatchList[i].counter))
					>= (WatchList[i].value & 0x0000FFFF)) {
				/* counter is higher than value, so heartbeat is not received in time */
				WatchList[i].status = NMT_STATE_INTIALIZING;
				CANopen_NMT_Send_CMD((WatchList[i].value >> 16) & 0x7F,
						NMT_CMD_RESET_COMM);
				CANopen_Heartbeat_Consumer_Failed(
						(WatchList[i].value >> 16) & 0x7F);
	...
	if (reInit_state) {
		if (reInit_state & 0x01) {
			...
			if (CANopen_SDOC_Exp_Write(reInit_ID, 0x1017, 0x00,
					(uint8_t*) reInit_buf[0], 2)) {
				reInit_state &= ~0x01;
				reInit_state |= 0x02;
			}
			...
			if (CANopen_SDOC_Exp_Write(reInit_ID, 0x1016, 0x01,
					(uint8_t*) reInit_buf[1], 4)) {
				reInit_state &= ~0x04;
				reInit_state |= 0x08;
			}
...
void CAN_RX(uint8_t msg_obj_num) {
	...
	if (msg_obj_num == 3) {
		...
		for (i = 0; i < WatchListLength; i++) {
			if ((CANopen_Msg_Obj.mode_id & 0x7F) == WatchList[i].NodeID
					|| (CANopen_Msg_Obj.mode_id & 0x7F)
							== ((WatchList[i].value >> 16) & 0x007F)) {
				...
				if (CANopen_Msg_Obj.data[0] == 0x00) {
					...
					CANopen_NMT_Consumer_Bootup_Received(
							CANopen_Msg_Obj.mode_id & 0x7F);
				}
			}
		}
	...
	if (msg_obj_num == 5) {
		...
		if (CANopen_Msg_Obj.data[1] == CAN_NODE_ID
	...
	if (msg_obj_num == 7) {
		...
		if (CANopen_SDOC_State == CANopen_SDOC_Exp_Read_Busy) {
			...
			if ((CANopen_Msg_Obj.data[0] & (7 << 5)) == 0x40) {
				...
				while (i--)
					CANopen_SDOC_Buff[i] =
							CANopen_Msg_Obj.data[CANopen_SDOC_InBuff++ + 4]; /* save valid databytes to memory */
			...
			if (((CANopen_Msg_Obj.data[0] & (7 << 5)) == 0x40)
					&& ((CANopen_Msg_Obj.data[0] & (1 << 1)) == 0x00)) {
				...
				for (i = 0; i < (7 - ((CANopen_Msg_Obj.data[0] >> 1) & 0x07));
						i++) {
					/* get all data from frame and save it to memory */
					if (CANopen_SDOC_InBuff < CANopen_SDOC_Seg_BuffSize)
						CANopen_SDOC_Buff[CANopen_SDOC_InBuff++] =
								CANopen_Msg_Obj.data[i + 1];
					else {
				...
				if (CANopen_Msg_Obj.data[0] & 0x01) {
					...
					CANopen_Msg_Obj.mode_id = 0x600 + CANopen_SDOC_Seg_ID;
					CANopen_Msg_Obj.data[0] = 0x60
							| ((CANopen_Msg_Obj.data[0] & (1 << 4)) ^ (1 << 4)); /* toggle */
					CANopen_Msg_Obj.data[1] = 0x00;
					CANopen_Msg_Obj.data[2] = 0x00;
					CANopen_Msg_Obj.data[3] = 0x00;
					CANopen_Msg_Obj.data[4] = 0x00;
			...
			if ((((CANopen_Msg_Obj.data[0] & (7 << 5)) == 0x60)
					&& ((CANopen_Msg_Obj.data[0] & (1 << 1)) == 0x00))
					|| ((CANopen_Msg_Obj.data[0] & (7 << 5)) == 0x20)) {
				...
				CANopen_Msg_Obj.data[0] = ((CANopen_Msg_Obj.data[0] & (1 << 4))
						^ (1 << 4)); /* toggle */
				...
				if (CANopen_SDOC_InBuff == CANopen_SDOC_Seg_BuffSize) {
					CANopen_Msg_Obj.data[0] |= ((7
							- (CANopen_SDOC_Seg_BuffSize % 7)) << 1) | 0x01; /* save length */
...
void CANopen_NMT_Send_CMD(uint8_t ID, uint8_t CMD) {
	static CAN_MSG_OBJ msg_NMT_TX = { 0x00, /* mode_id */
	0x00, /* mask */
	{ 0x00, /* data[0] */
	0x00, /* data[1] */
	0x00, /* data[2] */
...
void CANopen_NMT_Change_MyState(uint8_t NMT_Command) {
	...
	case NMT_CMD_ENTER_PRE_OP:
		...
		if (CANopen_NMT_MyState == NMT_STATE_INTIALIZING
	...
	case NMT_CMD_START:
		...
		if (CANopen_NMT_MyState == NMT_STATE_PRE_OPERATIONAL
				|| CANopen_NMT_MyState == NMT_STATE_STOPPED) {
			/* only change to this new state if this is allowed from previous state */
			CANopen_NMT_MyState = NMT_STATE_OPERATIONAL;
		}
...
void CANopen_Heartbeat_Send(void) {
	static CAN_MSG_OBJ msg_HeartbeatTX = { 0x700 + CAN_NODE_ID, /* mode_id */
	0x00, /* mask */
	{ 0x00, /* data[0] */
	0x00, /* data[1] */
	0x00, /* data[2] */
 ...
 ** Returned value:		Return 0 for successs, SDO Abort code for error
 *****************************************************************************/
uint32_t CANopen_SDOS_Exp_Write(uint16_t index, uint8_t subindex,
		uint8_t *dat_ptr) {
	return 0; /* Return 0 for successs, SDO Abort code for error */
}
 ...
 ** Returned value:		Return 0 for successs, SDO Abort code for error
 *****************************************************************************/
uint32_t CANopen_SDOS_Seg_Read(uint16_t index, uint8_t subindex,
		uint8_t openclose, uint8_t *length, uint8_t *data, uint8_t *last) {
 ...
 ** Returned value:		Return 0 for successs, SDO Abort code for error
 *****************************************************************************/
uint32_t CANopen_SDOS_Seg_Write(uint16_t index, uint8_t subindex,
		uint8_t openclose, uint8_t length, uint8_t *data, uint8_t *fast_resp) {
 ...
 ** Returned value:		none
 *****************************************************************************/
void CANopen_SDOS_Find_Buffer(uint16_t index, uint8_t subindex,
		SDOS_Buffer_t** SDOS_Buffer, uint8_t* access_type) {
	uint32_t i;
 ...
 *****************************************************************************/
uint8_t CANopen_SDOC_Exp_Read(uint8_t node_id, uint16_t index, uint8_t subindex,
		uint8_t* data, uint8_t* valid_data_bytes) {
 ...
 ** Returned value:		Returns 1 for success, 0 for error
 *****************************************************************************/
uint8_t CANopen_SDOC_Exp_Write(uint8_t node_id, uint16_t index,
 ...
 ** Returned value:		Returns 1 for success, 0 for error
 *****************************************************************************/
uint8_t CANopen_SDOC_Seg_Read(uint8_t node_id, uint16_t index, uint8_t subindex,
		void* buff, uint32_t buffSize) {
	CAN_MSG_OBJ msg_SDO_Seg_TX;
 ...
 ** Returned value:		Returns 1 for success, 0 for error
 *****************************************************************************/
uint8_t CANopen_SDOC_Seg_Write(uint8_t node_id, uint16_t index,
		uint8_t subindex, void* buff, uint32_t length) {
	CAN_MSG_OBJ msg_SDO_Seg_TX;
